...
// Search By Catagory
if ($status == 0 && $cat != 0) {
    // 1st time search by catogary (WHERE)
    $q .= " WHERE `catogary`.`catogary_id` = '" . $cat . "'";
    $status = 1;
} else if ($status != 0 && $cat != 0) {
    // 2st time search by catogary (AND)
    $q .= " AND `catogary`.`catogary_id` = '" . $cat . "'";
}
...
if ($num2 == 0) {
    // Serch No Result
?>
    <div class="d-flex  flex-column align-items-lg-center mt-5">
        <h5>Search No Result</h5>
        <p>We're Sorry, We Cannot find any matherd for your search Item..</p>
    </div>
    <?php
} else {
